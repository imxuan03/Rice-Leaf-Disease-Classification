...
model_path = "E:/Niên Luận/RICE LEAF DISEASES/CODE/backend/models/lenet_model30_multi_full_data.h5"
...
class RasaChatbotAPIView(APIView):
    def post(self, request, *args, **kwargs):
        # Lấy message từ yêu cầu của người dùng
        ...
        # Dữ liệu sẽ gửi đến Rasa
        payload = {
            "sender": "user",  # Tùy chỉnh ID người dùng nếu cần
        ...
        # Trả về phản hồi từ Rasa
        if response.status_code == 200:
            if response_data:
                return Response({"response": response_data[0]["text"]}, status=status.HTTP_200_OK)
            else:
                return Response({"response": "No response from Rasa"}, status=status.HTTP_200_OK)
        else:
            return Response({"error": "Failed to get a response from Rasa"}, status=response.status_code)